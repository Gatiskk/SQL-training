...
const insertMovieActors = (
  ...
  filteredActors.map(actors => `(${movieId}, ${actors.id})`).join(",");
};
...
const insertMovieDirectors = (
  ...
  filteredDirectors.map(directors => `(${movieId}, ${directors.id})`).join(",");
};
...
const insertMovieKeywords = (
  ...
  filteredKeywords.map(keywords => `(${movieId}, ${keywords.id})`).join(",");
};
...
const insertMovieProductionCompanies = (
  ...
  const filteredCompanies = productionCompanyRows.filter(company => productionCompanies.indexOf(company.company_name) != -1)
  return `insert into ${MOVIE_PRODUCTION_COMPANIES} (movie_id, company_id) values` +
  filteredCompanies.map(productionCompanies => `(${movieId}, ${productionCompanies.id})`).join(",");
};
